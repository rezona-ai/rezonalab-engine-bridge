# game-web 接入 Rezona Bridge

给 `rezonalab-game-web` 的接入说明：怎么依赖 `@rezonalab/engine-bridge-web`，一次连接与一次发送的完整代码，错误码到文案的映射，本地怎么用假引擎联调，以及入口面板与卡片必须遵守的交互规则。视觉与文案以会话产出的可点击原型为准（链接在 game-web PR 描述里）。
...
按钮与面板受**构建期**环境变量控制：`NEXT_PUBLIC_ENGINE_BRIDGE=1` 时渲染，否则整条功能不出现在页面上（不是运行时 kill switch）。回退方式是关掉该变量重新构建部署；插件是用户本机安装物，网页不渲染入口它就只是在 localhost 空转，无害。
...
// ── 入口面板：连接状态放在一个全局 store 里，整页只有一条连接 ──
type BridgeState =
  | { status: 'idle' }
...
export async function sendToEngine(
  ...
  if (state.status !== 'connected' || state.engine !== engine) {
    showToast('先在 Rezona Bridge 面板里连接该引擎'); // 未连接：只提示，不自动去连
    return;
  }
...
| `ORIGIN_REJECTED` | 连接 | 插件拒绝了本站来源 | 提示在插件「高级 → 额外允许来源」里加本站地址。**确定信号**：插件升级握手后关 4403，客户端据此与「没有引擎」区分开 |
...
| `DISCONNECTED` | 发送 | 插件掉线，传输中断 | 面板回到未连接，不自动重连 |
...
npm run dev:fake-engine -- --project-dir ./tmp-project-2 --name SecondGame --origin http://localhost:3000   # 再起一个 → 自动落 41701，面板应出现两个工程
...
- **不自动重连**：`onClose` 触发后只把面板回到未连接并给一条提示；由用户再拨开关。
- **未连接时点卡片只提示**：不代替用户去连，提示指向入口面板。
