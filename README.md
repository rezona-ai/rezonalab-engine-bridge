...
1. 用户在工作台画布左上角「Rezona Bridge」拨开某个引擎的开关，网页并行探测该引擎的端口段（Cocos `41700–41719`、Unity `41720–41739`），与第一个应答的插件握手并保持连接。
2. 在画布卡片上点「导出至 → 该引擎」，网页算好 sha256，按 4 MB 分块推字节，插件边收边写临时文件、校验、原子搬进 `assets/RezonaAssets/`，再导入资产库并实例化到场景。
