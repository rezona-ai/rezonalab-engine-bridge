...
插件读取 `Origin` 头，精确匹配（scheme + host + port）白名单；缺失或不匹配 → **放行 HTTP 101，随即以关闭码 4403 关闭**，
**不发** `hello_ack`、不读任何入向帧，也**绝不影响**已建立的合法连接。

之所以不在升级前用 HTTP 401 拒绝：浏览器的 WebSocket 接口拿不到握手阶段的响应状态码，
握手前拒绝会让「来源不在白名单」与「端口上没有插件」在网页侧完全同形（都是 1006），
用户只会看到「未找到编辑器」，无从知道要去插件「高级」里加来源。升级后关 4403 则让网页拿到确定信号。
代价是任何页面都能得知本机有插件在监听——但握手前拒绝也能靠时序问出同一个事实，所以这不是新增的暴露面。
